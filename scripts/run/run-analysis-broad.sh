...
# list for all targets without models in Eastern/Western analyses
if [ ! -e %OUTDIR%/qpAdm/westernModels/westernModels-all-models-with-p.tsv ] || [ ! -e %OUTDIR%/qpAdm/easternModels/easternModels-all-models-with-p.tsv ]; then
   echo "Need to run eastern and western models first"
   exit
fi
# determines samples without Eastern / Western models
western_leftovers=`Rscript $scriptDir/targets-with-no-models.R %OUTDIR%/qpAdm/westernModels/westernModels-all-models-with-p.tsv $pvalThres`
eastern_leftovers=`Rscript $scriptDir/targets-with-no-models.R %OUTDIR%/qpAdm/easternModels/easternModels-all-models-with-p.tsv $pvalThres`
# find samples that appear on both lists
targets=`echo $eastern_leftovers $western_leftovers | tr [:space:] "\n" | sort | uniq -c | tr -s " " | grep "^ 2 "| cut -d" " -f3`
